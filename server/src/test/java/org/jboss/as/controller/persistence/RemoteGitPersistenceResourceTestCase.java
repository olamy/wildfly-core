...
public class RemoteGitPersistenceResourceTestCase extends AbstractGitPersistenceResourceTestCase {
    ...
    public void createDirectoriesAndFiles() throws Exception {
        root = new File("target", "configuration").toPath();
        remoteRoot = new File("target", "remote").toPath().resolve("configuration");
    ...
    public void testDefaultPersistentConfigurationFile() throws Exception {
        ...
        GitRepository gitRepository = new GitRepository(root, remoteRoot.resolve(Constants.DOT_GIT).toAbsolutePath().toString(), null, null, Collections.emptySet());
