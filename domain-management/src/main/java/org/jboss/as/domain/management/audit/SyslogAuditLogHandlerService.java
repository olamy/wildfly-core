...
public class SyslogAuditLogHandlerService implements Service<SyslogAuditLogHandlerService>, SyslogCredentialReferenceSupplier {
    ...
    public static final ServiceName SYSLOG_AUDIT_HANDLER = ServiceName.of("org", "wildfly", "management", "audit", "handler");
    ...
    public static final SyslogAuditLogHandlerService installService(OperationContext context, String name, final Resource handlerResource) throws OperationFailedException {
        ...
        if (transport == SyslogAuditLogHandler.Transport.TLS) {
            ...
            for (ResourceEntry storeEntry : tlsStores) {
                ...
                if (type.equals(CLIENT_CERT_STORE)) {
                    if (storeModel.hasDefined(KeystoreAttributes.KEY_PASSWORD_CREDENTIAL_REFERENCE.getName())) {
                        service.tlsClientCertStoreKeyCredentialSourceSupplier
                                .inject(CredentialReference.getCredentialSourceSupplier(context, KeystoreAttributes.KEY_PASSWORD_CREDENTIAL_REFERENCE, storeModel, serviceBuilder));
                    }
                    if (storeModel.hasDefined(KeystoreAttributes.KEYSTORE_PASSWORD_CREDENTIAL_REFERENCE.getName())) {
                        service.tlsClientCertStoreCredentialSourceSupplier
                                .inject(CredentialReference.getCredentialSourceSupplier(context, KeystoreAttributes.KEYSTORE_PASSWORD_CREDENTIAL_REFERENCE, storeModel, serviceBuilder));
                    }
